<?php
	...
	if(isset($_POST['submit'])){//Get All value from Student registraion From 
		...
		if(empty($name)||empty($student_id)||empty($gender)||empty($dob)||empty($religion)||empty($email)||empty($roll)||empty($father_name)||empty($mother_name)||empty($father_job)||empty($father_phone)){
			...
			$query="insert into student (name,student_id,gender,dob,religion,nationality,address,email,phone,class,section,shift,admission_date,student_photo,father_name,mother_name,father_job,father_phone,father_photo,mother_job,mother_phone,password,alumni,status)
			values('$name','$student_id','$gender','$dob','$religion','$nationality','$address','$email','$phone','$class','$section','$shift','$admission_date','$destination','$father_name','$mother_name','$father_job','$father_phone','$father_destination','$mother_job','$mother_phone','','0','0')";
